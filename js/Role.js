var RoleFunc = function() {
	...
	function Character(index, characterDataInfo) {
		...
		this.updatetl = new TimelineLite({
			autoRemoveChildren : true
		});
		this.energytl = new TimelineLite({
			autoRemoveChildren : true
		});
		this.texttl = new TimelineLite({
			autoRemoveChildren : true
		});
		this.skilltl = new TimelineLite({
			autoRemoveChildren : true
		});
		...
		this.updateEnergyDisplay = function() {
			...
			this.energytl.to(this.energyIcon, ENERGY_UPDATE_TIME, {
				width : width
			});
		}
		...
		this.normalAttack = function() {
			...
			for (var i = 0; i < this.orbQueue.length; i++) {
				...
				if (i == 0) {
					this.updatetl.to(this.roleIcon, 0.1, {
						x : 0,
						y : -5
					}, "mylabel");
					this.updatetl.to(this.roleIcon, 0.1, {
					...
					this.updatetl.to(ballDiv, BALL_FLY_TIME, {
						visibility : "visible",
						bezier : {
							type : "thru",
							values : [{
								left : ballDiv.offsetLeft,
								top : ballDiv.offsetTop
							}, {
								left : centerPos,
								top : "-450px"
							}, {
						...
						onComplete : removeBallDiv,
						onCompleteParams : [charactersDiv, ballDiv, this, i]
					}, "-=0.1")
				} else {
		...
		this.addMainSkillAnimation = function(skillImg, text, skillID) {
			...
			this.skilltl.fromTo(skillDiv, 1, {
				...
				ease : Expo.easeIn,
				onComplete : removeSkillDiv,
				onCompleteParams : [$("#gameView")[0], skillDiv, skillID]
			});
		}
		...
		this.bindDisplay = function(mainDiv) {
			...
			if (this.color == "red") {
				this.rold_bk.style.backgroundImage = "url(image/fire_bk.png)";
				// change it
				this.rold_bk.style.backgroundSize = "cover";
				this.energyIcon.src = "image/fire_energy.png";
			} else if (this.color == "green") {
				this.rold_bk.style.backgroundImage = "url(image/forest_bk.png)";
				// change it
				this.rold_bk.style.backgroundSize = "cover";
				this.energyIcon.src = "image/forest_energy.png";
			} else if (this.color == "blue") {
				this.rold_bk.style.backgroundImage = "url(image/water_bk.png)";
				// change it
				this.rold_bk.style.backgroundSize = "cover";
